...
public class TeacherController {
    ...
    public String fileLoad(HttpServletRequest request, @RequestParam("filePhoto") MultipartFile file)
            throws IOException {
        if (!file.isEmpty()){
    ...
    public String objectLoad(HttpServletRequest request, @ModelAttribute User user, Model model) throws IOException {
        if (!user.getFilePhoto().isEmpty()){
            //1.path：为上传的目录，参数可以改为项目要求的(可改)
            String path = request.getServletContext().getRealPath("/upload/");
            //2.可以打印出，自己去检测是否正确(不可改)
            System.out.println(path);
            //3.查看原文件的名称(不可改)
            String fileName = user.getFilePhoto().getOriginalFilename();
            //4.根据目录和文件名，进行判断和创建(不可改)
            File filePath = new File(path,fileName);
            if (!filePath.getParentFile().exists()){
                filePath.getParentFile().mkdirs();
            ...
            user.getFilePhoto().transferTo(new File(path+File.separator+fileName));
